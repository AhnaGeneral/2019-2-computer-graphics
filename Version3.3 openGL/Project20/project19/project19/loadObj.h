...
int loadObj(const char* filename ,vector< glm::vec3>& returnvertex, vector< glm::vec3 >& returnnormal, vector< glm::vec2>& returnoutuv);
...
int loadObj(const char* filename, vector< glm::vec3 >&returnvertex, vector< glm::vec3 >& returnnormal, vector< glm::vec2>& returnoutuv)
{
	...
	for (unsigned int i = 0; i < vertexIndices.size(); i++) {
		...
		returnvertex.push_back(vertex);
	}
	for (unsigned int i = 0; i < uvIndices.size(); i++) {
		unsigned int uvIndex = uvIndices[i];
		glm::vec2 vertex = temp_uvs[uvIndex - 1];
		returnoutuv.push_back(vertex);
	}
	for (unsigned int i = 0; i < normalIndices.size(); i++) {
		unsigned int normalIndex = normalIndices[i];
		glm::vec3 vertex = temp_normals[normalIndex - 1];
		returnnormal.push_back(vertex);
	}

	return returnvertex.size();
}